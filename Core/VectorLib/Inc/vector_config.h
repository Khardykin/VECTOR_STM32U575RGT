...
/* --- РАБОЧЕЕ: источник времени для ВСЕХ таймингов VectorLib ---------------
 * Единственное место, где выбирается, откуда берётся время (vector_tick.h).
 * В коде модулей HAL_GetTick()/HAL_Delay() больше не встречается - только
 * VTICK_MS()/VTICK_SLEEP_MS()/VTICK_BUSYWAIT_MS().
 *   0 = HAL:    HAL_GetTick(), тайм-база TIM6, разрешение 1 мс. Тик даёт
 *               TIM6_IRQn с приоритетом 15 (ниже всех в проекте), поэтому при
 *               остановке ядра отладчиком или шквале прерываний он отстаёт.
 *   1 = RTOS:   tx_time_get(), тик ThreadX (SysTick 100 Гц, приоритет 4),
 *               разрешение 10 мс - грубее, но устойчивее к задержкам.
 * ThreadX тайм-базу HAL НЕ перенастраивает: TIM6 и SysTick - два независимых
 * таймера (подробно в docs/CODE_MAP.md, раздел 7).                          */
#ifndef VECTOR_TICK_SOURCE
#define VECTOR_TICK_SOURCE        0
#endif
...
/* Периодический отчёт о системном тике, секунд (0 = не печатать). Печатается
   из потока (lvgl_thread_entry), поэтому из ISR ничего не вызывается:
     tick 10 s: irq=+10000 hal=+10000 min=999 max=1042 us
   irq - сколько сработало прерываний тайм-базы (TIM6), hal - насколько за это
   же время вырос HAL_GetTick(). Оба должны давать +1000 за секунду и
   совпадать друг с другом. max_us показывает реальное дрожание: меряется по
   счётчику циклов ядра (DWT, 6.25 нс), а не по HAL_GetTick.                 */
