...
4. Или перевести весь VectorLib на тик RTOS: `VECTOR_TICK_SOURCE 1`. Он в этом
   логомере оказался в 34 раза здоровее TIM6, цена — разрешение 10 мс вместо
   1 мс (для антидребезга 50 мс, паузы цикла 500 мс и таймаутов SPI 250 мс
   этого достаточно).
...
Все модули VectorLib пользуются `vector_tick.h`, прямых `HAL_GetTick()`/
`HAL_Delay()` в них нет (остались только в `main.c` — там они и есть предмет
измерения, и в сгенерированных кубом файлах):

```c
uint32_t t0 = VTICK_MS();            /* текущее время, мс */
...
if (VTICK_ELAPSED_MS(t0) > 250u) ... /* прошло больше 250 мс, переполнение учтено */
VTICK_SLEEP_MS(100);                 /* сон потока, CPU не грузит */
VTICK_BUSYWAIT_MS(5);                /* только до старта планировщика */
```

Источник выбирается одним макросом `VECTOR_TICK_SOURCE`: `0` = HAL (TIM6,
1 мс), `1` = RTOS (`tx_time_get()`, SysTick, 10 мс). Переключение ничего в
коде модулей не меняет.
